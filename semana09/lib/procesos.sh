...
listar_top_cpu() {
    ...
    NR > 1 && NR <= n+1 {
        printf "%-10s PID:%-8s CPU:%-6s%% MEM:%-6s%% CMD:%s\n",
        $1, $2, $3, $4, $11
    }'
}
...
detectar_zombies() {
    ...
    if [[ -z "$zombies" ]]; then
        echo "Ningún proceso zombie detectado"
    else
        echo "$zombies" | while read -r pid ppid stat comm; do
            printf "PID:%-8s PPID:%-8s ESTADO:%-4s CMD:%s\n" \
            "$pid" "$ppid" "$stat" "$comm"
        done
    fi
}
...
arbol_usuario() {
    ...
    ps -u "$usuario" --forest -o pid,stat,comm
}
